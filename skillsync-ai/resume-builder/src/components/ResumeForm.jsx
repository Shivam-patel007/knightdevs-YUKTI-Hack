    ...
    <div className="rounded-3xl border border-white/20 bg-white/5 p-6 shadow-2xl backdrop-blur-2xl md:p-8">
      ...
      {currentStepId === 'experience' && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addExperience} className="rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/20">+ Add</button>
          </div>
              ...
              <div key={i} className="rounded-2xl border border-white/15 bg-white/5 p-5 backdrop-blur-xl">
                <div className="mb-4 flex justify-end">
                  {data.experience?.length > 1 && (
                    <button type="button" onClick={() => removeExperience(i)} className="text-sm text-red-300 hover:text-red-200">Remove</button>
                  )}
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                  <div><label className="mb-1.5 block text-sm text-white/70">Job title</label><input type="text" value={exp.title} onChange={(e) => updateExperience(i, 'title', e.target.value)} className={inputGlass} placeholder="Software Engineer" /></div>
                  <div><label className="mb-1.5 block text-sm text-white/70">Company</label><input type="text" value={exp.company} onChange={(e) => updateExperience(i, 'company', e.target.value)} className={inputGlass} placeholder="Acme Inc." /></div>
                  <div><label className="mb-1.5 block text-sm text-white/70">Start date</label><input type="text" value={exp.start} onChange={(e) => updateExperience(i, 'start', e.target.value)} className={inputGlass} placeholder="Jan 2020" /></div>
                  <div><label className="mb-1.5 block text-sm text-white/70">End date</label><input type="text" value={exp.end} onChange={(e) => updateExperience(i, 'end', e.target.value)} className={inputGlass} placeholder="Present" /></div>
                </div>
                <div className="mt-4">
                  <label className="mb-1.5 block text-sm text-white/70">Description / bullet points</label>
                  <textarea value={exp.description} onChange={(e) => updateExperience(i, 'description', e.target.value)} className={`${inputGlass} min-h-[100px] resize-y`} placeholder="Key responsibilities and achievements..." />
                </div>
              </div>
            ))}
          </div>
      ...
      {currentStepId === 'education' && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addEducation} className="rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/20">+ Add</button>
          </div>
          <div className="space-y-6">
              ...
              <div key={i} className="rounded-2xl border border-white/15 bg-white/5 p-5 backdrop-blur-xl">
                <div className="mb-4 flex justify-end">
                  {data.education?.length > 1 && (
                    <button type="button" onClick={() => removeEducation(i)} className="text-sm text-red-300 hover:text-red-200">Remove</button>
                  )}
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                  <div><label className="mb-1.5 block text-sm text-white/70">Degree</label><input type="text" value={edu.degree} onChange={(e) => updateEducation(i, 'degree', e.target.value)} className={inputGlass} placeholder="B.S. Computer Science" /></div>
                  <div><label className="mb-1.5 block text-sm text-white/70">School</label><input type="text" value={edu.school} onChange={(e) => updateEducation(i, 'school', e.target.value)} className={inputGlass} placeholder="University of Example" /></div>
                  <div><label className="mb-1.5 block text-sm text-white/70">Field (optional)</label><input type="text" value={edu.field} onChange={(e) => updateEducation(i, 'field', e.target.value)} className={inputGlass} placeholder="Computer Science" /></div>
      ...
      {currentStepId === 'skills' && (
        <div className="space-y-5">
          ...
          <textarea value={skillsText} onChange={(e) => updateSkills(e.target.value)} className={`${inputGlass} min-h-[140px] resize-y`} placeholder="JavaScript, React, Tailwind CSS, ..." />
        </div>
      )}
      ...
      {currentStepId === 'summary' && (
        <div className="space-y-5">
          ...
          <textarea value={data.summary ?? ''} onChange={(e) => update('summary', e.target.value)} className={`${inputGlass} min-h-[180px] resize-y`} placeholder="Experienced professional with..." />
        </div>
      )}

      <div className="mt-8 flex justify-between">
        <button type="button" onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0} className="rounded-xl border border-white/20 bg-white/10 px-5 py-2.5 text-white transition disabled:opacity-40 hover:bg-white/20 disabled:hover:bg-white/10">Previous</button>
        <button type="button" onClick={() => setStep((s) => Math.min(STEPS.length - 1, s + 1))} disabled={step === STEPS.length - 1} className="rounded-xl border border-white/30 bg-white/20 px-5 py-2.5 font-medium text-white shadow-lg transition disabled:opacity-40 hover:bg-white/30 disabled:hover:bg-white/20">Next</button>
      </div>
    </div>
  )
}
