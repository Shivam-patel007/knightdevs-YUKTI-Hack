 ...
 */
export async function POST_SKILLS(req: NextRequest) {
...
/**
 * Proxies POST /api/ats-match to the Node/Express ATS backend (port 5000).
 * Forwards the raw multipart body so file upload works correctly.
 */
export async function POST(req: NextRequest) {
  ...
  try {
    const body = await req.arrayBuffer();
    const res = await fetch(`${ATS_BACKEND_BASE}/api/ats-match`, {
      method: "POST",
      headers: {
        "Content-Type": contentType,
      },
